...
It consumes the wrapper from the repo root via a `file:../..` dependency, so it exercises the real published packaging path: Vite resolves `svelte-fusioncharts` through the package's `svelte` export condition to the uncompiled `src/index.svelte` source and compiles it with Svelte 5.
...
> The Svelte 4 example gallery lives in [`../svelte4-webpack`](../svelte4-webpack).
